...
\begin{marginfigure}
  \begin{tikzpicture}[>=latex, scale=5]
    ...
    \coordinate (teddyx) at (0.4, 0);
    \coordinate (teddyy) at (0, 0.6);
    \coordinate (teddy) at ($(teddyx) + (teddyy)$);
    \draw[->] (origin) -- (1, 0) node[right] {$x$};
    \draw[->] (origin) -- (0, 1) node[left] {$y$};
    \begin{scope}[shift={(teddy)}, scale=0.05]
      \teddybear
    \end{scope}
    \draw[very thick] (origin) -- node[above] {$r$} (teddy);
    \draw[dashed] (teddyx) node[below] {$x$} -- (teddy) -- (teddyy) node[left] {$y$};
    \draw (0.1, 0) arc (0:54:0.1);
    \node at (0.13, 0.07) {$\theta$};
  \end{tikzpicture}
\end{marginfigure}
...
\exemple{
  ...
  cartésiennes $(x, y) = (\SI{3}{\meter}, \SI{2}{\meter})$.  Déterminer les
  coordonnées polaires de cet objet.

  On trouve d'abord $r$ avec le théorème de Pythagore:
  \begin{align*}
    r &= \sqrt{x^2 + y^2} \\
      &= \sqrt{(\SI{3}{\meter})^2 + (\SI{2}{\meter})^2} \\
      &= \sqrt{\SI{14}{\meter\squared}} \\
      &= \SI{3.74}{\meter}
  \end{align*}
  Ensuite, on trouve l'angle.  Comme le point est dans le premier quadrant,
  l'angle sera nécessairement entre \num{0} et \SI{90}{\degree}.
  \begin{align*}
    \tan\theta &= \frac{y}{x} \\
               &= \frac{\SI{2}{\meter}}{\SI{3}{\meter}} \\
        \theta &= \arctan\left(\frac{2}{3}\right) \\
               &= \SI{33.7}{\degree}
  \end{align*}
  Le point en question est donc décrit par les coordonnées polaires
  $(r, \theta) = (\SI{3.74}{\meter}; \SI{33.7}{\degree})$.
}

\subsection{Exemple de conversion du système de coordonnées polaires au
  système de coordonnées cartésiennes}

\exemple{
  Supposons que la position d'un objet est donnée par les coordonnées
  polaires $(r, \theta) = (\SI{3}{\meter}, \SI{230}{\degree})$.  Déterminer les
  coordonnées cartésiennes de cet objet.

  Il suffit d'appliquer les définitions du sinus et du cosinus.
  \begin{align*}
    x &= r\cos\theta \\
      &= \SI{3}{\meter} \cos \SI{230}{\degree} \\
      &= \SI{-1.93}{\meter}
  \end{align*}
  \begin{align*}
    y &= r\sin\theta \\
      &= \SI{3}{\meter} \sin \SI{230}{\degree} \\
      &= \SI{-2.30}{\meter}
  \end{align*}
...
\subsection{Exemple d'analyse dimensionnelle}

\exemple{
  Essayons de déterminer une relation entre le temps de chute d'un corps à la
  surface de la Terre et la hauteur à partir de laquelle le corps tombe.

  Il est raisonnable de croire que le temps de chute dépendra de la hauteur
  initiale du corps, de sa masse et de l'accélération gravitationnelle.  Par
  contre nous ne savons pas si la dépendance est linéaire, quadratique, cubique
  ou autre.  Nous introduisons donc trois exposants, $\alpha$, $\beta$ et
  $\gamma$, ainsi qu'une constante sans dimension $k$.  La relation proposée
  est
  \[
    t = k m^\alpha h^\beta g^\gamma
  \]
  Pour trouver les valeurs des exposants, nous n'avons qu'à s'assurer que les
  dimensions sont les mêmes de part et d'autre de l'équation.  On a
  \begin{align*}
    [t] &= T \\
    [m] &= M \\
    [h] &= L \\
    [g] &= L/T^2
  \end{align*}
  Il n'y a pas de masse à gauche et ni la hauteur ni l'accélération
  gravitationnelle ne contiennent la dimension de masse.  Par conséquent,
  l'exposant de $m$ doit être $0$, i.e., la masse n'apparaît pas dans cette
  relation.  Pour que la longueur du côté droit disparaisse, il faut que
  \[
    \beta + \gamma = 0
  \]
  Pour obtenir une unité de temps, il faut que
  \[
    -2\gamma = 1
  \]
  Autrement dit, $\alpha = 0$, $\beta = 1/2$ et $\gamma = -1/2$.  La relation
  recherchée est donc
  \[
    t = k\sqrt{\frac{h}{g}}
  \]
  Une simple analyse dimensionnelle nous permet donc de déterminer que le temps
  de chute doit être proportionnel à la racine carrée de la hauteur.
}

\subsection{Exercice sur l'analyse dimensionnelle}

\exemple{
  En relativité restreinte, l'énergie $E$ d'un objet est reliée à sa masse $m$
  sa vitesse $v$, et la vitesse de la lumière $c$.  Les dimensions de ces
  quantités sont les suivantes
  \begin{align*}
    [E] &= ML^2/T^2 \\
    [m] &= M \\
    [v] &= L/T \\
    [c] &= L/T
  \end{align*}
  On considère aussi une constante sans dimension $\gamma$.

  Laquelle des relation suivantes est un candidat raisonnable pour la relation
  ...
  \begin{enumerate}
    ...
    \item $E^2 = \frac{1}{2} m^2v^2 + (\gamma mc)^2$
